 ...
 */
export default function GitHubStatsPhase({ accentColor, username, sectionIndex = 4 }: GitHubStatsPhaseProps) {
      ...
      <div className="max-w-4xl w-full">
        ...
        {loading ? (
          <div className="flex justify-center">
            <div className={`w-8 h-8 border-2 ${accentBorderClass} border-t-transparent rounded-full animate-spin`} />
          </div>
        ) : (
              ...
              <div key={weekIndex} className="flex flex-col gap-1">
                {Array.from({ length: 7 }, (_, dayIndex) => {
                  ...
                  return (
                    <div
                      key={dayIndex}
                      className={`w-2 h-2 md:w-3 md:h-3 rounded-sm ${bgClass}`}
                    />
                  );
                })}
              </div>
